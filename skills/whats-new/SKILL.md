...
2. Read the markdown output and prepend a 1–3 bullet TL;DR above it. Surface
   ...
   - Lines beginning with `> note:` — surface those; they explain
     degraded modes (offline, non-GitLab origin, missing identity, etc.).

   If nothing stands out, skip the TL;DR and let the sections speak for
   themselves.

3. Answer follow-up questions conversationally. `Bash(git:*)` and
   `Bash(glab:*)` are in `allowed-tools` for this step — use them directly
   for things like "show me the diff for that branch" or "what's in MR !1234"
   rather than re-running the summary script.
...
- On non-GitLab origins the script still produces sections 1, 2, and 4; do
  not pretend the GitLab sections "failed" — they were deliberately skipped.
